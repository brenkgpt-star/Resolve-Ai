    ...
    <section className="border-b-2 border-stone-900 px-4 sm:px-6 py-12">
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-7">
          <div>
            ...
            <p className="text-stone-600 text-sm sm:text-base max-w-xl mt-1">
              Escolha uma situação, descreva com suas palavras ou envie uma foto do local. Nossa IA analisa o defeito e monta a lista exata do que comprar.
            </p>
          </div>
          ...
          {!chipsVisible && (
            <button
              onClick={onReset}
              className="flex items-center gap-2 px-3 py-2 bg-stone-200 hover:bg-stone-300 text-stone-800 text-xs font-bold border-2 border-stone-900 shadow-neo-sm neo-btn shrink-0 w-fit transition-all cursor-pointer"
            >
              <RotateCcw size={13} />
        ...
        <div className="border-2 border-stone-900 bg-white shadow-neo">
          ...
          {/* Messages Feed */}
          <div className="flex flex-col gap-3.5 px-5 pt-6 pb-4 min-h-[160px] max-h-[420px] overflow-y-auto bg-stone-50/50">
            {messages.map((m, i) => (
              <ChatMessage
                key={i}
                who={m.who}
          ...
          {/* Freeform Input com Câmera e Upload de Imagens */}
          <FreeformInput
            onSend={onFreeText}
